...
δ = 2.0pp。依据是既有先例而不是为这次结果挑的：已发布的 int4 变体相对 bf16 的
损失点估计是 1.85pp，那就是本项目已经对外认可过的容差。

**这个依据本身有一处要如实标注**：int4 那 1.85pp 是在 logprob 口径下、用两个
独立比例的标准误报的，**从来没做过配对检验**（int4 是 bitsandbytes 格式，
vllm-ascend 载不了，没法在同一套官方口径下重测），所以它是点估计而不是置信上界。

**但发布判定不依赖 δ 的精确取值**：W8A8_DYNAMIC 在全量 26,368 题上的损失置信
上界是 0.93pp，**δ 取 [1.0pp, 3.0pp] 区间里任何值，结论都一样**。这条比争论
先例值多少更有说服力——**要检查的是结论对阈值敏不敏感，而不是把阈值论证得多完美。**
...
def main():
    ...
    if not a.control:
        ...
        # δ 敏感性：判定结果对阈值敏不敏感，比把阈值论证得多完美更重要。
        # 阈值是人定的，容易被事后挑成「刚好能过」；如果结论在一个宽区间里都不变，
        # 那就不用争 δ 到底该是 1.85 还是 2.00。
        grid = [0.005, 0.01, 0.015, 0.02, 0.025, 0.03]
        verdicts = [(g_, worst <= g_) for g_ in grid]
        print("    δ 敏感性：" + "  ".join(
            f"{g_*100:.1f}pp={'过' if v else '不过'}" for g_, v in verdicts))
        # 判定只在 δ == 损失上界处翻转，所以「安全边界」就是 worst 本身：
        # 任何 δ > worst 都过，任何 δ < worst 都不过。真正要看的是 worst 离
        # 我们声明的 δ 有多远，以及它有没有落在「合理容差」区间里面。
        REASONABLE = (0.01, 0.03)      # 1~3pp：量化变体的通行容差范围
        if worst <= REASONABLE[0]:
            print(f"    → 损失上界 {worst*100:.2f}pp 低于合理容差区间下沿 "
                  f"{REASONABLE[0]*100:.1f}pp，**δ 取该区间内任何值都过**，"
                  f"结论不依赖阈值怎么定")
        elif worst >= REASONABLE[1]:
            print(f"    → 损失上界 {worst*100:.2f}pp 高于合理容差区间上沿 "
                  f"{REASONABLE[1]*100:.1f}pp，**δ 取该区间内任何值都不过**")
        else:
            print(f"    → ⚠️ 损失上界 {worst*100:.2f}pp 落在合理容差区间 "
                  f"{REASONABLE[0]*100:.1f}~{REASONABLE[1]*100:.1f}pp **里面**，"
                  f"判定就是 δ 定在它两边哪一侧的问题。")
            print(f"       这种情况下不要靠调 δ 定输赢——要么把 CI 收窄"
                  f"（加大 n），要么在报告里明确写出这个变体处在容差边界上。")
