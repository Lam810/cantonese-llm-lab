...
**呢份筆記刻意保留咗我自己出過嘅錯同撤回**（撤回過「排第一」、定錯兩次閘門阈值、
喺 8 行嘅表上看 5 行就下全稱結論……）。負結果同更正記錄同正面結果一樣有用。
...
> ⚠️ **09-16 版本嘅首頁寫「我們排第一」，係錯嘅，已撤回。** 上面用嘅係我哋自己嘅
> logprob 口徑。按 **HKMMLU 官方口徑**（嚴格 zero-shot prompting、生成式）將 8 個模型
> 全量重跑之後，**HKMMLU 我哋第 4、書面粵語純度第 2**。下面全部係官方口徑。
...
我喺 8 個模型裡跑完 5 個嗰陣寫咗「那句腳手架只幫我哋」——補齊之後係錯嘅，
基座同 Thinking 版受益都比我哋大。缺嘅那 3 行正好係行為最極端嘅三個
