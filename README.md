...
在一項出現退化嘅訓練中，loss 喺 200 步之內由 1.59 跌到 0.25，然後 6400 步走平——表面上呈現收斂趨勢，
...
但存在兩個錯誤訊息與實際根因不匹配嘅問題：缺 **NNAL/ATB**（`libatb.so`，唔喺 CANN 裡面、要單獨 source），
...
**判斷一個口徑能唔能用，要看它對所有被比較對象是否同等中立，而唔係只看表面公平性。**
應報告**排名對提示詞嘅穩定性**。
詳見 [`results/prompt_ablation.md`](results/prompt_ablation.md)。

**9. 顯著性唔可以當容差門檻——n 越大越容易顯著。**
同一個量化變體，3300 題測出 −1.48pp（p=0.025，顯著），全量 26,368 題測出
−0.46pp（p=0.052，唔顯著）。如果規則係「唔顯著才發」，會產生與樣本量相關嘅反常決策。
