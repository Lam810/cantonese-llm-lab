...
在一项出现退化的训练中，loss 在 200 步内从 1.59 降到 0.25，然后 6400 步走平——表面上呈现收敛趋势，
...
但存在两个错误信息与实际原因不匹配的问题：缺 **NNAL/ATB**（`libatb.so`，不在 CANN 里、要单独 source），
...
**判断一个口径能不能用，要看它对所有被比较对象是否同等中立，而不是只看表面公平性。**
应报告**排名对提示词的稳定性**。
详见 [`results/prompt_ablation.md`](results/prompt_ablation.md)。

**9. 显著性不能当容差门槛——n 越大越容易显著。**
同一个量化变体，3300 题测出 −1.48pp（p=0.025，显著），全量 26,368 题测出
−0.46pp（p=0.052，不显著）。如果规则是「不显著才发布」，会产生与样本量相关的反常决策。
