...
**这份笔记刻意保留了我自己犯过的错和撤回**（撤回过「排第一」、定错两次闸门阈值、
在 8 行的表上看 5 行就下全称结论……）。负结果和更正记录与正面结果一样有用。
...
> ⚠️ **09-16 版本的首页写「我们排第一」，是错的，已撤回。** 那里用的是我们自己的
> logprob 口径。按 **HKMMLU 官方口径**（严格 zero-shot prompting、生成式）把 8 个模型
> 全量重跑之后，**HKMMLU 我们第 4、书面粤语纯度第 2**。下面全部是官方口径。
...
我在 8 个模型里跑完 5 个时写了「那句脚手架只帮我们」——补齐之后是错的，
基座和 Thinking 版受益都比我们大。缺的那 3 行正好是行为最极端的三个
